...
u8 Capture_Usability(const struct MenuItemDef *def, int number)
{
	...
	for (i = 0; i < ARRAY_COUNT_RANGE1x1; i++)
	{
		...
		if ((gActiveUnit->conBonus + gActiveUnit->pClassData->baseCon + gActiveUnit->pCharacterData->baseCon) <= (unit_enemy->conBonus + unit_enemy->pClassData->baseCon + unit_enemy->pCharacterData->baseCon))
			continue;
