...
#if defined(SID_AstraStorm) && (COMMON_SKILL_VALID(SID_AstraStorm))
u8 AstraStorm_Usability(const struct MenuItemDef * def, int number)
{
...
int AstraStorm_Hover(struct MenuProc * menu, struct MenuItemProc * item)
{
...
int AstraStorm_Unhover(struct MenuProc * menu, struct MenuItemProc * menuItem)
{
    HideMoveRangeGraphics();
    return 0;
}

u8 AstraStorm_OnSelected(struct MenuProc * menu, struct MenuItemProc * item)
{
    if (item->availability == MENU_DISABLED)
    {
        MenuFrozenHelpBox(menu, MSG_MenuSkill_AstraStorm_FRtext);
        return MENU_ACT_SND6B;
    }

    gActionData.unk08 = SID_AstraStorm;
...
bool Action_AstraStorm(ProcPtr parent)
{
