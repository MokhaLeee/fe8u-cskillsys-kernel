...
STATIC_DECLAR NOINLINE void BattleGenerateHitHpCostCombatArt(struct BattleUnit *attacker, struct BattleUnit *defender)
{
	...
	case CID_BloodTribute:
		...
		AddBattleHpCost(
			...
			k_udiv(
				k_udiv(attacker->hpInitial * SKILL_EFF0(SID_COMBAT_BloodTribute), 100)
	...
	case CID_CrimsonStrike:
		...
		AddBattleHpCost(
			...
			GetCurrentBattleHitRound(),
			k_udiv(
				k_udiv(attacker->hpInitial * SKILL_EFF0(SID_COMBAT_CrimsonStrike), 100)
	...
	case CID_VitalReckoning:
		...
		AddBattleHpCost(
			...
			GetCurrentBattleHitRound(),
			k_udiv(
				k_udiv(attacker->hpInitial * SKILL_EFF0(SID_COMBAT_VitalReckoning), 100)
