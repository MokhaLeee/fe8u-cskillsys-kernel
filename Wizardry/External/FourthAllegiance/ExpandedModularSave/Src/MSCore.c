...
struct ChapterState;  // Forward declaration
...
void MS_CopyGameSave(int sourceSlot, int targetSlot) {
	...
	sbm.magic32 = SBM_MAGIC1_GAME;
	sbm.kind    = SAVE_TYPE_GAME;
...
void MS_SaveGame(unsigned slot) {
	...
	// Clear suspend
	InvalidateSuspendSave(SAVE_BLOCK_SUSPEND);

	// Update save slot index
	// 	gChapterData.saveSlotIndex = slot;
	gPlaySt.save_menu_type = slot;
