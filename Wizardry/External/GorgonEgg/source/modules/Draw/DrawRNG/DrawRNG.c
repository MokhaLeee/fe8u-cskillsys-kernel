...
struct Vec2 DrawRNG_TilemapDestination(struct PlayerInterfaceProc* proc, struct Vec2 coords)
{
...
const u8 gDrawRNGExtendLUT[] = {GE_WIDTH / 2, (GE_WIDTH * 3) / 4, (GE_WIDTH * 9) / 10, GE_WIDTH};


void DrawRNG_Extend(struct PlayerInterfaceProc* proc)
{
  ...
  width = gDrawRNGExtendLUT[proc->movementFrame];
  ...
  if (proc->movementFrame == (sizeof(gDrawRNGExtendLUT) / sizeof(s8)))
  {
    proc->busyFlag = FALSE;
...
// This has slightly different numbers than vanilla in order to
// accommodate windows of many sizes.
const u8 gDrawRNGRetractLUT[] = {(GE_WIDTH * 3) / 4, GE_WIDTH / 2, 0};


void DrawRNG_Retract(struct PlayerInterfaceProc* proc)
{
  ...
  width = gDrawRNGRetractLUT[proc->movementFrame];
  ...
  if (proc->movementFrame == (sizeof(gDrawRNGRetractLUT) / sizeof(s8)))
  {
